...
const ApiCard = ({ api, onClick }) => {
    ...
    return (
        <div
            onClick={() => onClick(api)}
            className="group relative bg-slate-900 border border-slate-800 rounded-2xl p-5 hover:border-indigo-500/50 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all cursor-pointer flex flex-col h-full"
        >
            ...
            <h3 className="text-lg font-bold text-white mb-1 group-hover:text-indigo-400 transition-colors truncate" title={api.name}>
                {api.name}
            </h3>
            ...
            <div className="pt-4 border-t border-slate-800 flex items-center justify-between text-xs font-medium text-slate-500">
                ...
                <div className="flex items-center space-x-1 group-hover:text-indigo-400 transition-colors">
